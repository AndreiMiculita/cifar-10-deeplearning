...
# from src.utils import load_state_dict_from_url
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, activation, inplanes, planes, stride=1, downsample=None, groups=1,
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        layers = []
        layers.append(block(self.activation, self.inplanes, planes, stride, downsample, self.groups,
...
def _resnet(activation, arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(activation, block, layers, **kwargs)
    # if pretrained:
...
def resnet18(activation, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet(activation, 'resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
